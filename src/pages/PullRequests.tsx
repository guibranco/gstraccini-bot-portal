...
const stateOrder = ['success', 'failure', 'pending', 'error', 'skipped', ''] as const;

export function PullRequests() {
  const [isLoading, setIsLoading] = useState(true);
  const [pullRequests, setPullRequests] = useState<PullRequest[]>([]);
  const [groupedPullRequests, setGroupedPullRequests] = useState<GroupedPullRequests>({});
  ...
  const stateOptions = stateOrder.map(state => ({
    ...
    color: state === 'success' ? '22c55e' :
           state === 'failure' ? 'ef4444' :
           state === 'pending' ? 'eab308' :
           state === 'error' ? 'ef4444' :
           state === 'skipped' ? '6b7280' : '6b7280'
  }));
    ...
    const fetchPullRequests = async () => {
      try {
        ...
        const mockPullRequests: PullRequest[] = [
          {
            ...
            labels: [
              ...
              { name: 'security', color: 'dc3545', description: 'Security related' }
            ],
            sender: {
              login: 'alice',
              name: 'Alice Johnson',
          ...
          },
          {
            ...
            owner: 'Organization 1',
            labels: [
              { name: 'bug', color: 'dc3545', description: 'Bug fix' }
            ],
            sender: {
              login: 'bob',
              name: 'Bob Smith',
        ...
        const uniqueLabels = Array.from(
          new Set(
            mockPullRequests.flatMap(pr => pr.labels || [])
        ...
        const filteredPRs = mockPullRequests.filter(pr => {
          if (selectedOwners.length > 0 && !selectedOwners.includes(pr.owner || 'Unknown')) return false;
          if (selectedSenders.length > 0 && !selectedSenders.includes(pr.sender.login)) return false;
          if (selectedStates.length > 0 && !selectedStates.includes(pr.state)) return false;
          if (selectedLabels.length > 0) {
            const prLabels = pr.labels?.map(l => l.name) || [];
            if (!selectedLabels.some(label => prLabels.includes(label))) return false;
          }
          return true;
        });
  ...
  const ownerOptions = Array.from(new Set(pullRequests.map(pr => pr.owner || 'Unknown'))).map(owner => ({
    value: owner,
    label: owner,
  }));

  const senderOptions = Array.from(
    new Set(pullRequests.map(pr => pr.sender))
      .values()
  ).map(sender => ({
    value: sender.login,
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          ...
          <span className="px-3 py-1 bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-100 rounded-full text-sm font-medium">
            {Object.values(groupedPullRequests).reduce((acc, curr) => acc + curr.length, 0)}
          </span>
        </div>

        {Object.entries(groupedPullRequests).map(([owner, ownerPullRequests]) => (
          <PullRequestGroup
            key={owner}
            owner={owner}
