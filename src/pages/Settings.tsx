...
import type {
  ...
  TrustedSender,
  DependabotFrequency,
  RepositorySettings as RepoSettings,
} from "../types";
...
export function Settings() {
  ...
  const showSuccessMessage = useCallback(
    (message: string = "Settings updated successfully") => {
  ...
  const handleRepositorySettingChange = useCallback(
    async (key: keyof RepoSettings, value: boolean | DependabotFrequency) => {
      setSettings((prev) => ({
        ...prev,
        repository: {
          ...prev.repository,
  ...
  const handleSettingChange = useCallback(
    async (key: keyof Settings, value: boolean | number) => {
  ...
  const handleIntegrationSettingChange = useCallback(
    async (integration: string, key: string, value: boolean | number) => {
      setSettings((prev) => ({
        ...prev,
        integrations: {
          ...prev.integrations,
  ...
  const handleAddTrustedSender = useCallback(
    async (login: string) => {
      try {
  ...
  const handleRemoveTrustedSender = useCallback(
    async (login: string) => {
      try {
        ...
        setTrustedSenders((prev) =>
          prev.filter((sender) => sender.login !== login),
        );
